"""GET /api/projects/{id} — loads a single project by PROJECT_ID."""
...
def run(parameters: list, messages: object) -> ResponseModel:
    try:
        ...
        response = ProjectService(user_email=user_email, portal_url=portal_url, token=token).get_project_by_id(project_id)
        _emit(response)
        return response
    except Exception as exc:
        logger.exception("Unhandled error in GetProjectById.")
...
class GetProjectByIdTool:
    def __init__(self) -> None:
        self.label             = "Get Project By ID"
        self.description       = "Loads a single project by PROJECT_ID."
